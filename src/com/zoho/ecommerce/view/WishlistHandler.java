...
public class WishlistHandler implements Execute, Creatable, Editable, Viewable, Deletable {
    ...
    public void add() {
        ...
        sc.nextLine();
        if (new ProductController().isProductExistCard(card.getProduct(), product.getId())) {
    ...
    public void update() {
        ...
        if (cardProduct.canAddToCard() && newQuantity > 0) {
            ...
            System.out.println("✅ Product Quantity updated successfully: " + cardProduct.getProduct().getProductName() + " 📦 Updated Quantity: " + newQuantity);
        } else {
...
// checking the card product quantity is availble or out of stock
    public void checkQuantityExist(List<CardProduct> cardProduct) {
        for (CardProduct obj : cardProduct) {
            Product products = new ProductController().isProductExist(obj.getProduct().getId());
            if (products != null) {
                if (products.getStock() < obj.getQuantity()) {
                    if (products.getStock() > 0) {
                        System.out.println("⚠️ " + obj.getProduct().getProductName() + " Stock Available: " + products.getStock() + " Your Chosen Quantity: " + obj.getQuantity());
                        obj.setQuantity(products.getStock());
                        System.out.println("✅ Available Stock " + products.getStock() + " Updated to " + obj.getProduct().getProductName());
                    } else {
                    ...
                    System.out.println("Product Choosed :" + obj.getProduct().getProductName() + " Quantity: " + obj.getQuantity() + " Price " + obj.getProduct().getPrice());
                }
            } else {
                System.out.println("❌ The Product " + obj.getProduct().getProductName() + " is not available.");
                cardProduct.remove(obj);
                System.out.println("🗑️ It has been removed from the cart.");
